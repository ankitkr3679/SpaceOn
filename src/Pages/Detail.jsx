...
const PROPERTIES = Array.from({ length: 12 }).map((_, i) => {
    ...
    return {
        ...
        sub:
            type === "Yard"
                ? "Heavy Vehicle Access • Power Ready"
                : "High-Clear Height • Loading Bay",
        location:
            i % 2 ? "Zada Tower, Business Bay, Dubai" : "The Crest, Sobha Hartland",
        image: imgs[0],
        images: imgs,
        agency: "Better Homes",
        rating: 4.5 + (i % 3) * 0.1,
...
const Pill = ({ children, active = false, onClick, icon: Icon, badge }) => (
    <button
        type="button"
        onClick={onClick}
        className={`px-3 py-1.5 rounded-md text-sm border transition inline-flex items-center gap-2 ${active
            ? "bg-[#020A64] text-white border-[#020A64]"
            : "bg-white text-gray-700 border-[#020A64] hover:bg-gray-50"
            }`}
    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-[minmax(280px,38%)_1fr] auto-rows-auto">
                    ...
                    <div className="relative md:h-auto flex items-start">
                        ...
                        <button
                            ...
                            title="Save"
                            onClick={(e) => e.preventDefault()} // ✅ prevents triggering Link when clicking Save
                        >
                            <FaHeart />
                        </button>
...
export default function ListingsPage() {
    ...
    const baseList = useMemo(() => {
        ...
        if (activeArea)
            list = list.filter((_, idx) => (idx + activeArea.length) % 2 === 0);
        return list;
    ...
    const filtered = useMemo(() => {
        ...
        if (quick === "Warehouse")
            return baseList.filter((p) => p.type === "Warehouse");
        return baseList;
    }, [baseList, quick]);

    return (
        <main className="min-h-screen bg-gray-50">
            {/* 👇 Match the navbar inner width */}
            <div className="w-[93%] mx-auto py-6 sm:py-7 md:py-8 space-y-6">
                {/* Title & subtitle */}
                <div className="flex flex-col gap-1">
                    ...
                    <div className="text-xs sm:text-sm text-gray-600 flex flex-wrap items-center gap-2 sm:gap-3">
                        ...
                        <span className="inline-flex items-center gap-1">
                            <FaFilter /> Yard:{" "}
                            <strong>{yardCount.toLocaleString()}</strong>
                        </span>
                        <span className="inline-flex items-center gap-1">
                            <FaWarehouse /> Warehouse:{" "}
                            <strong>{warehouseCount.toLocaleString()}</strong>
                        </span>
                        {activeArea && (
                            <span className="inline-flex items-center gap-1">
                                <FaMapMarkerAlt /> in{" "}
                                <strong className="truncate">{activeArea}</strong>
                            </span>
                        )}
                        {showVerified && (
                            <span className="inline-flex items-center gap-1">
                ...
                <div className="sm:hidden flex items-center gap-2 text-sm text-gray-700">
                    ...
                    <Toggle
                        ...
                        label="Show verified first"
                    />
                </div>

                {/* Areas (aligned with navbar width) */}
                <div className="flex flex-row flex-wrap gap-2 overflow-x-auto">
                    {AREAS.map((a) => (
                        <Pill
                            ...
                            onClick={() =>
                                setActiveArea(activeArea === a.name ? null : a.name)
                            }
                            icon={FaMapMarkerAlt}
                            badge={a.count}
                        >
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Pill
                        active={quick === "Warehouse"}
                        onClick={() => setQuick("Warehouse")}
                        icon={FaWarehouse}
                ...
                <div className="flex flex-wrap items-center justify-center gap-2 pt-4">
                    ...
                    {[1, 2, 3].map((n) => (
                        <button
                            key={n}
                            className={`px-3 py-2 rounded-lg border ${n === 1
                                ? "bg-[#020A64] text-white border-[#020A64]"
                                : "bg-white text-[#020A64] hover:bg-gray-50"
                                }`}
